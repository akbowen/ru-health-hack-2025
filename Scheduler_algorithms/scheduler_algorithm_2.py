...
contract_df = pd.read_excel('ru-health-hack-2025/sample-data/Provider contract.xlsx')
...
credentialing_df = pd.read_excel('ru-health-hack-2025/sample-data/Provider Credentialing.xlsx')
...
coverage_df = pd.read_excel('/ru-health-hack-2025/sample-data/Facility Coverage.xlsx')
...
volume_df = pd.read_excel('ru-health-hack-2025/sample-data/Facility volume.xlsx')
...
availability_df = pd.read_excel('ru-health-hack-2025/sample-data/Provider Availability.xlsx')
...
# ============================================================================
# WEEKEND CONSTRAINT AUDIT
# ============================================================================

print("="*80)
print("WEEKEND CONSTRAINT AUDIT")
print("="*80)

# Weekend days in October 2025: [4, 5, 11, 12, 18, 19, 25, 26]
print(f"\nWeekend days: {WEEKEND_DAYS}")

# ============================================================================
# 1. CHECK WEEKEND SHIFT COUNT COMPLIANCE
# ============================================================================
print("\n" + "-"*80)
print("Checking P1: Weekend Shift Count Compliance")
print("-"*80)

violations = []

for provider in providers_to_schedule:
    assigned_weekends = providers_to_schedule[provider]['assigned_weekends']
    allowed_weekends = providers_to_schedule[provider]['weekend_shifts']

    if assigned_weekends > allowed_weekends:
        violations.append((provider, assigned_weekends, allowed_weekends))

if violations:
    print(f"\n⚠️  VIOLATIONS FOUND: {len(violations)}")
    for provider, assigned, allowed in violations[:10]:
        print(f"  {provider}: {assigned} assigned > {allowed} allowed")
    if len(violations) > 10:
        print(f"  ... and {len(violations) - 10} more")
else:
    print(f"✓ No violations - all providers within weekend limits")

# ============================================================================
# 2. WEEKEND COVERAGE DISTRIBUTION
# ============================================================================
print("\n" + "-"*80)
print("Weekend Coverage Distribution")
print("-"*80)

weekend_slots_assigned = len([s for s in slots if s['is_weekend'] and s['status'] == 'ASSIGNED'])
weekend_slots_total = len([s for s in slots if s['is_weekend']])

print(f"Weekend slots assigned: {weekend_slots_assigned}/{weekend_slots_total} ({weekend_slots_assigned/weekend_slots_total*100:.2f}%)")

# By weekend day
for day in sorted(WEEKEND_DAYS):
    day_slots = [s for s in slots if s['day'] == day]
    day_assigned = len([s for s in day_slots if s['status'] == 'ASSIGNED'])
    print(f"  Day {day}: {day_assigned}/{len(day_slots)} ({day_assigned/len(day_slots)*100:.2f}%)")

# By shift type on weekends
print(f"\nWeekend slots by shift type:")
md1_weekend = len([s for s in slots if s['is_weekend'] and s['shift'] == 'MD1' and s['status'] == 'ASSIGNED'])
md2_weekend = len([s for s in slots if s['is_weekend'] and s['shift'] == 'MD2' and s['status'] == 'ASSIGNED'])
pm_weekend = len([s for s in slots if s['is_weekend'] and s['shift'] == 'PM' and s['status'] == 'ASSIGNED'])

md1_weekend_total = len([s for s in slots if s['is_weekend'] and s['shift'] == 'MD1'])
md2_weekend_total = len([s for s in slots if s['is_weekend'] and s['shift'] == 'MD2'])
pm_weekend_total = len([s for s in slots if s['is_weekend'] and s['shift'] == 'PM'])

print(f"  MD1: {md1_weekend}/{md1_weekend_total}")
print(f"  MD2: {md2_weekend}/{md2_weekend_total}")
print(f"  PM: {pm_weekend}/{pm_weekend_total}")

# ============================================================================
# 3. PROVIDER WEEKEND UTILIZATION
# ============================================================================
print("\n" + "-"*80)
print("Provider Weekend Utilization")
print("-"*80)

print(f"\nFT Providers - Weekend assignments:")
for provider in sorted(ft_providers):
    assigned = providers_to_schedule[provider]['assigned_weekends']
    allowed = providers_to_schedule[provider]['weekend_shifts']
    utilization = (assigned / allowed * 100) if allowed > 0 else 0
    print(f"  {provider}: {assigned}/{allowed} ({utilization:.1f}%)")

print(f"\nIC Providers (first 10) - Weekend assignments:")
for provider in sorted(ic_providers)[:10]:
    assigned = providers_to_schedule[provider]['assigned_weekends']
    allowed = providers_to_schedule[provider]['weekend_shifts']
    utilization = (assigned / allowed * 100) if allowed > 0 else 0
    print(f"  {provider}: {assigned}/{allowed} ({utilization:.1f}%)")
...
# ============================================================================
print("\n" + "="*80)
print("WEEKEND CONSTRAINT SUMMARY")
print("="*80)

total_weekend_assigned = sum(providers_to_schedule[p]['assigned_weekends'] for p in providers_to_schedule)
total_weekend_allowed = sum(providers_to_schedule[p]['weekend_shifts'] for p in providers_to_schedule)

print(f"\nOverall weekend shift compliance:")
print(f"  Total assigned: {total_weekend_assigned}/{total_weekend_allowed}")
print(f"  Utilization: {total_weekend_assigned/total_weekend_allowed*100:.2f}%")

if violations:
    print(f"\n⚠️  STATUS: VIOLATIONS EXIST - Weekend constraints NOT fully respected")
else:
    print(f"\n✓ STATUS: All weekend constraints satisfied")

days = range(1, 32)
facilities = coverage["Facility"].unique()
shifts = ["MD1", "MD2", "PM"]

# --- Step 1: Create blank schedule ---
columns = [f"{facility}-{shift}" for facility in facilities for shift in shifts]
final_schedule = pd.DataFrame(index=days, columns=columns)

# --- Step 2: Fill schedule ---
for facility in facilities:
    facility_data = coverage[coverage["Facility"] == facility]
    for shift in shifts:
        col_name = f"{facility}-{shift}"
        for day in days:
            # Get coverage info
            cov_row = facility_data[facility_data["Shift"] == shift]
            if cov_row.empty:
                final_schedule.loc[day, col_name] = "BLACK"
                continue

            coverage_status = str(cov_row["Coverage"].iloc[0]).strip().lower()

            # No coverage needed
            if coverage_status == "none" or coverage_status == "nan":
                final_schedule.loc[day, col_name] = "BLACK"
                continue

            # Check for assignment
            assigned = assigned_shifts[
                (assigned_shifts["Facility"] == facility)
                & (assigned_shifts["Shift"] == shift)
                & (assigned_shifts["Day"] == day)
            ]

            if not assigned.empty:
                final_schedule.loc[day, col_name] = assigned["Provider"].iloc[0]
            else:
                final_schedule.loc[day, col_name] = "RED"

# --- Step 3: Export to Excel ---
output_path = "ru-health-hack-2025/sample-data/Final_Schedule_algo_2.xlsx"
final_schedule.index.name = "Day"
final_schedule.to_excel(output_path)

# --- Step 4: Apply color formatting (and hide RED/BLACK text) ---
...
for row in ws.iter_rows(min_row=2, min_col=2):  # skip header
    for cell in row:
        if cell.value == "RED":
            cell.fill = red_fill
            cell.value = ""  # hide text
        elif cell.value == "BLACK":
            cell.fill = black_fill
            cell.font = white_font
            cell.value = ""  # hide text

wb.save(output_path)
print(f"✅ Final colored and formatted schedule saved as: {output_path}")

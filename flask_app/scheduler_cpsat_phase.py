...
class Scheduler:
    ...
    def run(self):
        ...
        path_min = self._export_with_score(sol_min, "1_minimize_providers", slots, score_min)
        path_bal = self._export_with_score(sol_bal, "2_balanced_providers", slots, score_bal)
        path_cons = self._export_with_score(sol_cons, "3_phase1_conservative", slots, score_cons)
        ...
        variations = [
        ('Minimize', score_min, path_min),
        ('Balanced', score_bal, path_bal),
        ('Conservative', score_cons, path_cons),
         ]
        ranked = sorted(variations, key=lambda x: x[1]['satisfaction_score'])
        return {
        "paths": {
            "minimize": path_min,
            "balanced": path_bal,
            "conservative": path_cons,
        },
        "ranking": [
            # list of dicts in order: rank 1, rank 2, rank 3
            {"name": name, "score": s['satisfaction_score'], "path": p}
...
def run_scheduler_with_files(
    *,
    ...
    """
    Execute BaseScheduler + Scheduler using provided Excel files.
    Returns a summary dict; schedules are written to `output_dir`.
    """
    os.makedirs(output_dir, exist_ok=True)

    # Build base schedule from coverage + availability
    base = BaseScheduler(
        coverage_file=coverage_path,
        availability_file=availability_path,
        output_dir=output_dir,
    )
    base_file, coverage, shifts, facilities, days, dow_map = base.create_base_schedule()

    # Run CP-SAT + Phase 2 variants
    sched = Scheduler(
        base_file, coverage, shifts, facilities, days,
        availability_file=availability_path,
        contract_file=contract_path,
        credentialing_file=credentialing_path,
        volume_file=facility_volume_path,
    ...
    rank2_path = None
    if result.get("ranking") and len(result["ranking"]) >= 2:
        rank2_path = result["ranking"][1]["path"]
    ...
    produced = [f for f in expected if os.path.exists(os.path.join(output_dir, f))]
    missing  = [f for f in expected if f not in produced]

    return {
        "status": "ok",
        "output_dir": os.path.abspath(output_dir),
        "produced": list(result["paths"].values()),
        "ranking": result["ranking"],
        "rank2_path": rank2_path,
    }
# ============================================================================
# MAIN
# ============================================================================
